...
app=FastAPI(title="Resume Optimization Agent",version="2.0.0"); settings=get_settings(); app.add_middleware(CORSMiddleware,allow_origins=settings.allowed_origins,allow_credentials=True,allow_methods=["*"],allow_headers=["*"])
...
async def limit_api_requests(request, call_next):
    """Enforce a three-requests-per-ten-seconds API limit."""
...
def startup():
    global oracle_pool; oracle_pool=get_pool()
def public(item):
...
def decision(sid:str,payload:ReviewRequest,user:str=Depends(current_user),gemini_api_key:str|None=Header(default=None,alias="X-Gemini-API-Key")):
    ...
    except Exception as exc: raise HTTPException(502,f"Session could not continue: {exc}")
